...
\end{align*}
Since $[|W|\op C]=\mathbb{1}_\mathbb{K}$ iff for a given choice of $\op$, $|W|\op C$ is true and $\mathbb{0}_\mathbb{K}$ otherwise, \eqref{eq:Q1-prov} becomes
...
Let $(K,\oplus,\otimes,\mathbb 0,\mathbb 1,\ominus)$ be an idempotent commutative $m$-semiring.
...
\]
Fix a finite set $U$ and provenance $(\alpha)_{x\in U}\in K$.
For $W\subseteq U$ define
\[
...
T_U(W)
=
A_W\ominus\Bigl(\bigl(\bigoplus_{x\in U'\setminus W}A_{W\cup\{x\}}\bigr)\oplus(A_W\otimes\beta)\Bigr)
=
T_{U'}(W)\ominus(A_W\otimes\beta).
\end{equation}

\smallskip\noindent\emph{Now we show $F_C(U)\le F_C(U')\oplus(F_{C-1}(U')\otimes\beta)$.}
