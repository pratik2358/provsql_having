...
\[
  U \;=\; \{\, (a,\#2) \mid (a,\#2) \in q \,\}.
\]
  That is, \(U\) is a set (\textcolor{red}{We want it to be a multiset. For now I am just considering it to be a set}) of tuples in \(q\) corresponding to group-key \(a\).
...
\end{equation}
\\
Fix a subset \(W \subseteq U\) such that $|W|\op C$. Let, 
\[
A=\bigotimes_{u\in U}\alpha_u \; \text{and} \; B=\bigoplus_{u\in U \setminus W} \alpha_u
\]
...
\[
A \otimes B = A \otimes \Bigl( \bigoplus_{u \in U \setminus W} \alpha_u  \Bigr)=\bigoplus_{u \in U \setminus W} A \otimes \alpha_u=\bigoplus_{u\in U \setminus W}  \otimes_{w\in W \cup \{u \}} \alpha_w
\]
Therefore, 
\[
A \ominus (A\otimes B)
...
\bigotimes_{u\in W} \alpha_u
\;\ominus\;
\bigoplus_{u \in U \setminus W}
\otimes_{w \in W \cup \{u\}} \alpha_w .
\]
So, 
\[
\langle Q^{\op C}_1 \rangle^{\hat I}(a)
=
  \bigoplus_{\substack{W \subseteq U, \\ |W| \op C}}
\Bigl(
\bigotimes_{u \in W} \alpha_u
\;\ominus\;
\bigoplus_{u \in U \setminus W}
