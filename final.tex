...
Fix $W\subseteq U$ with $|W|=c$ and write
\[
R_W := \bigoplus_{x\in U\setminus W}\mathbf{x}^{(K)}.
\]
For each $u\in U\setminus W$,
\[
A_{W\cup\{u\}} = A_W \otimes \mathbf{u}^{(K)}.
\]
Right-distributivity of $\otimes$ over $\ominus$ gives
\[
A_W \ominus A_{W\cup\{u\}}
= A_W \ominus(A_W\otimes\mathbf{u}^{(K)})
= A_W \otimes (\mathbb{1}_K \ominus \mathbf{u}^{(K)}).
\tag{$\dagger$}
\]
...
&= \bigoplus_{u\in U\setminus W}
      \bigl(A_W \ominus A_{W\cup\{u\}}\bigr)
      \\[2pt]
&= \bigoplus_{u\in U\setminus W}
      A_W\otimes(\mathbb{1}_K \ominus \mathbf{u}^{(K)})
      \qquad\text{(by $(\dagger)$)}
      \\[2pt]
&= A_W \otimes
    \Bigl(\bigoplus_{u\in U\setminus W}
           (\mathbb{1}_K \ominus \mathbf{u}^{(K)})
    \Bigr).
\end{aligned}
\tag{$\ddagger$}
\]
...
Applying (I) again,
\[
\bigoplus_{u\in U\setminus W}(\mathbb{1}_K \ominus \mathbf{u}^{(K)})
=
\Bigl(\bigoplus_{u\in U\setminus W}\mathbb{1}_K\Bigr)
\;\ominus\;
\Bigl(\bigoplus_{u\in U\setminus W}\mathbf{u}^{(K)}\Bigr)
=
\mathbb{1}_K \ominus R_W,
\]
since $\bigoplus_{u}\mathbb{1}_K=\mathbb{1}_K$ by idempotence.

Substituting this into $(\ddagger)$ yields
\[
A_W \ominus \bigoplus_{u\in U\setminus W} A_{W\cup\{u\}}
=
...
Putting this into $(\ast)$ and summing over all $|W|=c$ gives
\[
S_c \ominus S_{c+1}
= \bigoplus_{\substack{W\subseteq U\\|W|=c}} t_W.
\]
