...
Proposition~\ref{prop:algorithms} rests on this collapse: it suffices to
enumerate the minimal worlds, polynomially many for a fixed threshold.
\end{example}
...
time $O(N\cdot S)$ for $\text{\sql{SUM}}(t)\op C$ with $t(u_i)\in\NN$
and $S\defeq\sum_{i=1}^N t(u_i)$, and in time $O(N)$ for
