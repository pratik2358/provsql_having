...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \REQUIRE an array $A \leftarrow \mathrm{supp}(\mathdutchcal{u})$ of weights, an array $G$ of gates with $|G| = |A|$, a target $C \in \mathbb{N}$
    \ENSURE list $\mathcal{R}$ of subsets of $G$ whose weights sum to $C$
    \STATE Initialize $dp[0] \leftarrow \{\emptyset\}$ \COMMENT{list containing the empty subset}
    \FOR{$j \gets 1$ \TO $C$}
      \STATE $dp[j] \leftarrow \emptyset$ \COMMENT{$dp[j]$: all gate-subsets realizing sum $j$}
    \ENDFOR
    \FOR{$i \gets 1$ \TO $|A|$}
      \STATE $w \gets A[i]$, $g \gets G[i]$
