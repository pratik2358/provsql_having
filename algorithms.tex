...
For other aggregate functions, using brute-force enumeration of non-empty worlds $W\subseteq [|U^{\preceq}_{\angsem{\hat I}{q'},\vec v}|]$ will always give the correct answer by our semantics.
...
\begin{algorithm}
  ...
  \Input{a finite set of group-member occurrences $U \defeq U^{\preceq}_{\angsem{\hat I}{q'},\vec v}$, an arbitrary ordering $\preceq$ of $U$, a term $t$ of max-index $\le k$ such that $t(U_r[u])\in\NN$ for all $r \in [|U|]$, a constant $C\in\NN$, and an operator $\op\in\{=,>,<,\ge,\le,\neq\}$}
 ...
 \BlankLine
  Consider the sequence $(U_r)^{\preceq}_{r \in [|U|]}$ ordered by $\preceq$\\
  $T \gets \mathrm{agg}_{t,f_\Sigma}([|U|]) = \sum_{r=1}^{|U|} t(U_r[u])$\;
...
Fix a $\mathbb K$-instance $\hat I$, a group key $\vec v$ and an arbitrary ordering $\preceq$. Let $U\defeq U^{\preceq}_{\angsem{\hat I}{q'},\vec v}$; then $(U_r)^{\preceq}_{r\in[|U|]}$ is the $\preceq$-ordered sequence of $U$. 
...
\{\,W\subseteq [|U|]\mid W\neq\emptyset,\mathrm{agg}_{t,f_\Sigma}(W)\op C\,\}.
\]
where $f_\Sigma$ is the \sql{SUM} aggregate, so that $\mathrm{agg}_{t,f_\Sigma}(W)=\sum_{r\in W}t(U_r[u])$.
\end{theoremrep}

\begin{appendixproof}
Write
\[
  T\defeq \mathrm{agg}_{t,f_\Sigma}([|U|])=\sum_{r=1}^{|U|} t(U_r[u]).
\]
...
For each case below, we show that the provenance for $\vec v$ can be computed in time polynomial in $|U^{\preceq}_{\angsem{\hat I}{q'},\vec v}|$. Since the queries are fixed, this implying polynomial time in data complexity.
...
$W\subseteq [|U^{\preceq}_{\angsem{\hat I}{q'},\vec v}|]$ such that $|W|\ge k\text{ or }|W|>k$, respectively.
...
which is polynomial in $|U^{\preceq}_{\angsem{\hat I}{q'},\vec v}|$. Therefore the provenance can be computedi n polynomial time.
...
$W\subseteq [|U^{\preceq}_{\angsem{\hat I}{q'},\vec v}|]$ such that $\sum_{r\in W} t((U^{\preceq}_{\angsem{\hat I}{q'},\vec v})_r[u])\ge c\text{ and }\sum_{r\in W} t((U^{\preceq}_{\angsem{\hat I}{q'},\vec v})_r[u])>c$ .
...
Therefore the number of minimal satisfying worlds is bounded by $\sum_{i=1}^{k}\binom{|U^{\preceq}_{\angsem{\hat I}{q'},\vec v}|}{i}$ ,
