...
The full listing of this algorithm is presented in Algorithm~\ref{alg:sum_dp} of the \hyperref[appendix:algorithms]{Appendix}.
...
The full listing of this algorithm is presented in Algorithm~\ref{alg:count_enum} of the \hyperref[appendix:algorithms]{Appendix}.
...
In an absorptive m-semiring in which $\otimes$ distributes over
$\ominus$, the possible-world sum then collapses to a closed form
combining at most two running $\oplus$-sums over the occurrences of the
group \lean{HavingMinMax}{Having.minScan_correct}
\lean{HavingMinMax}{Having.maxScan_correct}
\lean{HavingMinMax}{Having.firstScan_correct}, computed by a single scan
using $O(|U|)$ semiring operations. Both hypotheses are needed:
absorptivity fails in the tropical m-semiring over
...
\sql{MIN(a)}${}\ge c$ on a two-occurrence group
\lean{Semirings/Tropical}{TropicalR.minScan_ne_prov}; and distributivity
of $\otimes$ over $\ominus$ is what identifies the factored world
annotation with its expanded form, an identification that fails in
absorptive m-semirings without it
\lean{Semirings/ChainFive}{ChainFive.not_world_bound}.
...
Therefore the number of minimal satisfying worlds is bounded by $\sum_{i\le k+1}\binom{|U|}{i}$
\lean{Having}{Having.card_minimal_sum_ge_le},
