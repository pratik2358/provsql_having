...
Fix a $\mathbb K$-instance $\hat I$, an aggregate-free query $q'$ of arity~$k$, a group key $\vec v$ and an arbitrary ordering $\preceq$. Let $U\defeq\nobreak U^{\preceq}_{\angsem{\hat I}{q'},\vec v}=\nobreak((u_r,\alpha_r))_{r\in[|U|]}$ be the $\preceq$-ordered sequence of group-member occurrences.
...
\smallskip\noindent\textbf{Case 1: $\text{\sql{MIN(a)}}\op c$, $\text{\sql{MAX(a)}}\op c$, and $\text{\fakesql{PICKFIRST}\texttt{(a)}}\op c$.}
...
Take the condition
$\text{\sql{SUM(a)}}\ge 5$. The valid non-empty worlds are $\{1,2\}$,
...
Example~\ref{ex:poisson-binomial} in the appendix works out the
recurrence on the three contributors of Example~\ref{ex:semantics}.
\begin{toappendix}
