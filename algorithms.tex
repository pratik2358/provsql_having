...
\begin{algorithm}
  ...
  \Input{a dictionary $A \leftarrow \mathrm{supp}(\mathdutchcal{u})$, a constant $C \in M$, and an operator $\mathrm{op}\in\{=,>,<,\ge,\le\}$}
  \Output{a list $\mathcal{R}$ of subsets of $A$, s.t.\ $\forall R \in \mathcal{R}$, $\ \sum_{\mathbf{x}\in R}\mathbf{x}^{(M)}\ \mathrm{op}\ C$}
...
\begin{algorithm}
  ...
  \Input{dictionary $A\leftarrow\mathrm{supp}(\mathdutchcal{u})$, constant $C\in M$, operator $\mathrm{op}$, and probabilities $p(g)$ for all $g\in\mathcal{G}$}
  \Output{$\Pr\big([\mathdutchcal{u}\oslash C]_{\mathrm{op}}\big)$}

  $\mathcal{R} \gets \textsc{DP-Subsets}(A,C,\mathrm{op})$\tcp*[r]{Algorithm above}
  $P \gets 0$\;
