...
For other aggregate functions, using brute-force enumeration of non-empty worlds $W\subseteq [|U^{\hat I}_{q',\vec v}|]$ will always give the correct answer by our semantics.
...
For each case below, we show that the provenance for $\vec v$ can be computed in time polynomial in $|U^{\hat I}_{q',\vec v}|$. Since the queries are fixed, this implying polynomial time in data complexity.
...
The provenance can be computed by a direct scan over the occurrences in the group, using only polynomially many operations in $|U^{\hat I}_{q',\vec v}|$. Therefore these cases are in polynomial time for arbitrary m-semirings.
...
\end{itemize}
Since $k$ is fixed, the number of such worlds is $\binom{|U^{\hat I}_{q',\vec v}|}{k}\text{ or }\binom{|U^{\hat I}_{q',\vec v}|}{k+1}$,
which is polynomial in $|U^{\hat I}_{q',\vec v}|$. Therefore the provenance can be computedi n polynomial time.
...
$W\subseteq [|U^{\hat I}_{q',\vec v}|]$ such that $\sum_{r\in W} t((U^{\hat I}_{q',\vec v})_r[u])\ge c\text{ and }\sum_{r\in W} t((U^{\hat I}_{q',\vec v})_r[u])>c$ .
...
Let $W$ be a minimal satisfying world. If each selected occurrence contributes at least $c/k$, then the size of $W$ is at most $k$: indeed, if $|W|>k$, then the sum of the values in $W$ is already forced by more than $k$ non-negative contributions each at least $c/k$, contradicting minimality of $W$. Therefore every minimal satisfying $W$ has cardinality at most $k$.

Therefore the number of minimal satisfying worlds is bounded by $\sum_{i=1}^{k}\binom{|U^{\hat I}_{q',\vec v}|}{i}$ ,
which is polynomial in $|U^{\hat I}_{q',\vec v}|$, $k$ being fixed. Therefore the provenance can be computed in polynomial time.
...
Since $k$ is fixed, the number of such worlds is bounded by
  $\binom{|U^{\hat I}_{q',\vec v}|}{k},\, \sum_{i=1}^{k}\binom{|U^{\hat I}_{q',\vec v}|}{i},\text{ and } \sum_{i=1}^{k-1}\binom{|U^{\hat I}_{q',\vec v}|}{i}$, respectively. Each of these bounds is polynomial in $|U^{\hat I}_{q',\vec v}|$. Therefore the provenance can be computed in polynomial time in arbitrary m-semirings.
