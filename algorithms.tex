...
The following two theorems state that \textsc{ValidSum} and \textsc{ValidCount} return exactly the set $\{W\sqsubseteq U\mid W\neq\emptyset,\ \mathrm{agg}_{t,f}(W)\op C\}$ of valid worlds, i.e., the index set of the $\oplus$-sum of Definition~\ref{def:semantics}; the predicate provenance is then the $\oplus$-sum of $\ann_U(W)$ over the returned worlds.
...
\sql{MIN(a)}${}\op c$ has the closed form
...
\end{array}
\]
for instance, \sql{MIN(a)}${}<c$ holds in a world exactly when some
occurrence with $a_i<c$ is present, and absorptivity reduces the sum over
these worlds to the sum over the singletons, whereas \sql{MIN(a)}${}\ge c$
additionally requires that no occurrence with $a_i<c$ be present, whence
the factor $\mathbb 1\ominus L$. The closed form for \sql{MAX(a)}${}\op c$
is the mirror image, exchanging $<$ and~$>$
\lean{HavingMinMax}{Having.maxScan_correct}, and for
\fakesql{PICKFIRST}\texttt{(a)}${}\op c$ (the non-commutative aggregate of
...
Therefore the number of minimal satisfying worlds is bounded by
$\sum_{i\le k}\binom{|U|}{i}$ for \sql{SUM(a) >= c}
\lean{Having}{Having.card_minimal_sum_ge_le} and by
$\sum_{i\le k+1}\binom{|U|}{i}$ for \sql{SUM(a) > c}, which is
polynomial in $|U|$, $k$ being fixed. Therefore the provenance can be computed in polynomial time.
...
\smallskip\noindent\textbf{Case 5: \sql{SUM(a) = }$c$, \sql{SUM(a) <= }$c$, and \sql{SUM(a) < }$c$ with all values nonzero and $\frac ca\le k$, in arbitrary m-semirings.}
By correctness of Algorithm~\ref{alg:sum_dp}
(Theorem~\ref{thm:sumdp-correct}), the valid worlds are exactly the
...
Finally, for \sql{COUNT(*)}${}=k$, $\le k$ or $<k$ (Case~4), the valid worlds are exactly those of cardinality at most $k$, again polynomially many, so no absorptivity is needed; likewise for \sql{SUM(a)}${}=c$, $\le c$ or $<c$ when all values are at least $\frac ck$ (Case~5).
\end{proofsketch}
