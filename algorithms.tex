...
The algorithm works also for the special case where $t=1$, i.e., \sql{COUNT(*)}${}\op C$ comparisons.
...
For \sql{COUNT(*)}${}\op C$ we only rely on the fact that the result of a \sql{COUNT} depends only on the number of occurrences of the group key values of the selected tuples.
...
Fix a $\mathbb K$-instance $\hat I$, an aggregate-free query $q'$ of arity~$k$, a group key $\vec v$ and an arbitrary ordering $\preceq$. Let $U\defeq U^{\preceq}_{\angsem{\hat I}{q'},\vec v}=((u_r,\alpha_r))_{r\in[|U|]}$ be the $\preceq$-ordered sequence of group-member occurrences.
...
Fix a $\mathbb K$-instance $\hat I$, an aggregate-free query $q'$, a group key $\vec v$, an ordering $\preceq$, $C\in\NN$ and $\op\in\{=,>,<,\ge,\le,\neq\}$. Let $U\defeq U^{\preceq}_{\angsem{\hat I}{q'},\vec v}$. Let $\mathcal W$ be the output of \textsc{ValidCount} on input $(U,C,\op)$.
Then
\[
\mathcal W
=
