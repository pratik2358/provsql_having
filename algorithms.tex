...
For these aggregates, validity of a world is determined directly from the occurrences in $U$:
\begin{itemize}
...
The provenance can be computed by a direct scan over the occurrences in the group, using only polynomially many operations in $|U|$. Therefore these cases are in polynomial time for arbitrary m-semirings.
...
Therefore the number of minimal satisfying worlds is bounded by $\sum_{i=1}^{k+1}\binom{|U|}{i}$ ,
which is polynomial in $|U|$, $k$ being fixed. Therefore the provenance can be computed in polynomial time.
...
Since $k$ is fixed, the number of such worlds is bounded by
  $\binom{|U|}{k},\, \sum_{i=1}^{k}\binom{|U|}{i},\text{ and } \sum_{i=1}^{k-1}\binom{|U|}{i}$, respectively. Each of these bounds is polynomial in $|U|$. Therefore the provenance can be computed in polynomial time in arbitrary m-semirings.
...
For \sql{MIN(a)}, \sql{MAX(a)} and \fakesql{PICKFIRST}\texttt{(a)} (Case~1), the validity of a world is decided occurrence by occurrence, so the provenance is obtained by a single scan, in any m-semiring.
For \sql{COUNT(*)}${}\ge k$ or ${}>k$ (Case~2) and \sql{SUM(a)}${}\ge c$ or ${}>c$ (Case~3), the valid worlds form an upward-closed family, whose provenance, in an \emph{absorptive} m-semiring, equals that of its minimal worlds \lean{Having}{Having.absorbing_subfamily}. These minimal worlds have bounded cardinality: $k$, or $k+1$ for the strict variants (using the hypothesis $c/a\le k$ for \sql{SUM}); so there are only $O(|U|^{k+1})$ of them.
Finally, for \sql{COUNT(*)}${}=k$, $\le k$ or $<k$ (Case~4), the valid worlds are exactly those of cardinality at most $k$, again polynomially many, so no absorptivity is needed.
\end{proofsketch}

\begin{example}\label{ex:prop-simplification}
  Consider as in Example~\ref{ex:semantics} a group of three occurrences,
  this time with attribute values
$a(u_1)=3$ and $a(u_2)=a(u_3)=2$, and annotated in the Boolean-function
semiring $\mathbb B[X]$ by distinct variables $x_1,x_2,x_3$.
Take the condition
\sql{SUM(a)}${}\ge 5$. The valid non-empty worlds are $\{1,2\}$,
$\{1,3\}$ and $\{1,2,3\}$, and the possible-world semantics sums their
annotations:
\[
(x_1\land x_2\land\lnot x_3)\lor(x_1\land x_3\land\lnot x_2)
\lor(x_1\land x_2\land x_3).
\]
This family of worlds is upward-closed, so the argument of Case~3
applies: as $\mathbb B[X]$ is absorptive, the $\ominus$-factors are
redundant, and the provenance equals the $\oplus$-sum over the two
\emph{minimal} valid worlds $\{1,2\}$ and $\{1,3\}$ alone,
\[
(x_1\land x_2)\lor(x_1\land x_3)=x_1\land(x_2\lor x_3).
\]
Proposition~\ref{prop:algorithms} rests on this collapse: rather than the
(in general exponentially many) valid worlds, it suffices to enumerate
the minimal ones -- here those of size at most~$2$, and polynomially many
for any fixed threshold -- giving a polynomial-time computation.
\end{example}
