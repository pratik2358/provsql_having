...
T_W&:=\ A_W \otimes \bigl(\mathbb{1}_{\mathbb{K}} \ominus B_W\bigr)\\ 
...
\end{align*}
such that, $\ann^{\hat I}_{q,(a)}(W)=T_W.$
...
=
\bigoplus_{\substack{W\subseteq [|U^{\hat I}_{q,(a)}|],\\ W\neq\emptyset,\\|W|\op C}} T_W.
\end{equation}
\smallskip\noindent\emph{Provenance of the join queries:}
For every $W\subseteq [|U^{\hat I}_{q,(a)}|] $ and $C\ge 1$ we define,
\[
S_C \;:=\; \bigoplus_{\substack{W\subseteq [|U^{\hat I}_{q,(a)}|] \\|W|=C}} A_W
\quad
(\text{with }S_C=\mathbb{0}_\mathbb{K}\text{ if }C>|U|).
\]
...
join conditions $\#2<^*\#4<^*\cdots$) refined with a tie-breaking occurence attribute as in $U^{\hat I}_{q,(a)}$ so that it is possible to select distinct tuple occurences with equal second attributes in an increasing order. Thus, complying with multiset semantics, this corresponds to choosing a
