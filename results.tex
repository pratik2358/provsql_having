...
\end{proof}
We state a bounding lemma that we later use in the proof to show that summing $T_U(W)$ over all $W\supseteq V$ always dominates the product $A_V$ for $V\subseteq U$.
...
\bottomrule
\end{tabular}
\caption{Groups and annotations}
\label{tab:group_table}
\end{table}

We show that there exists a $\mathbb{K}$-instance $\hat I$ such that $\langle Q^{\op C}_1\rangle^{\hat I}\neq \langle Q^{\op C}_2\rangle^{\hat I}$.
We consider a query $q$ that returns groups which have 2 tuples. From the relation
shown in Table~\ref{tab:group_table}, the provenance of the group $g_1$ following our semantics is 
$(t_1 \otimes t_2)\otimes(\mathbb{1}\ominus t_3) \oplus (t_1 \otimes t_3)\otimes(\mathbb{1}\ominus t_2)\oplus(t_2 \otimes t_3)\otimes(\mathbb{1}\ominus t_1)$.

We consider the why-semiring provenance of $t_1, t_2, t_3$ are respectively $\{\{a\}\}, \{\{b\}\}, \{\emptyset\}$.
Now we consider the why-semiring provenance of the group $g_1$. The provenance of the group $g_1$ following our 
semantics is 
\begin{align*}
...
Now following the join semantics the provenance becomes:
\begin{align*}
...
We refer to the same relation shown in Table~\ref{tab:group_table} as in the proof of (ii). 
The provenance of the same group $g_1$ following the self-join query is $(t_1 \otimes t_2) \oplus (t_1 \otimes t_3) \oplus (t_2 \otimes t_3)$.

In $\mathbb{N}$-semiring, the first provenance evaluates to 0 while the second evaluates to $3$.
\end{proof}
