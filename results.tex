...
Since \(q\) is an arity~$2$ query, every tuple in its output has the form
\((\#1,\#2)\).
...
aggregate-comparison predicate \([\mathcal{a} \op C]\).
...
\Bigl(
  ...
  \chi_{\op}\bigl(f_\Sigma(\pi_{\#1}(((U^{\hat I}_{q',\vec v})_r[u])_{r\in W}^\omega)), B\bigr)
\Bigr)
=
\bigoplus_{\substack{W\subseteq [|U^{\hat I}_{q',\vec v}|]\\ W\neq\emptyset}}
\Bigl(
...
\Bigr),
\]
where $q'$ is the subquery before the implicit \sql{GROUP BY} on all tuples in $R$ with group key $\vec v=()$ and $\omega$ is any arbitrary ordering since our aggregate here is commutative.
