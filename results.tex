...
$
F_C([|U^{\hat I}_{q,(a)}|])\ :=\ \bigoplus_{\substack{W\subseteq [|U^{\hat I}_{q,(a)}|]\\|W|\ge C}} T_{[|U^{\hat I}_{q,(a)}|]}(W).
$
So,
...
\end{appendixproof}
\begin{proofsketch}
For any idempotent (follows from absorptivity) $m$-semiring $\mathbb K$ we first obtain a weak bound on $\mathbb K$-element(s) implying joint tuple-presence in corresponding \emph{possible-worlds}.
...
Using Lemma~\ref{lem:upward-expansion} we claim that $\mathbb K$-provenance expressions arising from provenance evaluation using the possible-world semantics in Section~\ref{sec:semantics} for queries that involve $\ge$-comparisons between \sql{COUNT} aggregates and some constant $C\in \mathbb{N}_{\ge 1}$, can be expressed as recurrences on C, given that $\mathbb K$ satisfies certain $m$-semiring properties.
...
as required.
\end{appendixproof}
We also formalize a standard recurrence for the natural \sql{JOIN}-based provenance expression, for such queries.
