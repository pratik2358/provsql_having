...
NP-hard problem. Take a finite multiset $M=\{\{m_1,\dots,m_n\}\}$ of $n$ natural integers and a
  ...
  $R(v)$ containing $n$ tuples $(m_1), \dots, (m_n)$, each tuple $t_i$
  being annotated with provenance $\mathbb{1}_\mathbb{K}$.

  We fix the query
  \begin{minted}{sql}
SELECT DISTINCT 1 FROM R HAVING SUM(v) = B
  \end{minted}
...
\Bigl(
  \mathbb{1}\otimes
\chi_{\op}\bigl(\Sigma(\vals_t(W)), B)\bigr)
\Bigr)=
  \bigoplus_{\substack{W\msubseteq U^{\hat I}_{q',\vec v}\\ W\neq\emptyset}}
\Bigl(
\chi_{\op}\bigl(f(\vals_t(W)), B)\bigr)
\Bigr)
\]
which is non-zero if and only if one of the term of the $\oplus$ ssum is non-zero since
