...
\Bigl(
  ...
  \chi_{\op}\bigl(f_\Sigma(\pi_{\#1}(((U^{\hat I}_{q',\vec v})_r[u])_{r\in W})), B)\bigr)
\Bigr)
=
\bigoplus_{\substack{W\subseteq [|((U^{\hat I}_{q',\vec v})_r)|]\\ W\neq\emptyset}}
\Bigl(
...
\Bigr),
\]
where $q'$ is the subquery before the implicit \sql{GROUP BY} on all tuples in $R$ with group key $\vec v=()$.
