...
\begin{definition}
    (Join Rewrite) For queries of the form $\mathsf{HAVING\; COUNT(*) = c}$ over a relation we define the join semantics as below:
\paragraph{Equal.}
\[
...
\otimes
  \chi_{=}\bigl(  {\bigplus_M}_{W\cap\mathrm{supp}(\mathdutchcal{u})} \mathbf{x}^{(M)}, c \bigr)
\;\otimes\;
  \Biggl(\mathbb{1}_K \ominus \Biggl( \bigoplus_{(\mathbf{X}\setminus W)\cap\mathrm{supp}(\mathdutchcal{u})}\mathbf{x}^{(K)} \Biggr)\Biggr)
\Biggr)
\]
...
\paragraph{Greater than equal.}

\[
[\mathdutchcal{u} \oslash c]_{\geq}=
\bigoplus_{W,|W| = c} 
\Biggl(
  \Bigl( \bigotimes_{W\cap\mathrm{supp}(\mathdutchcal{u})} \mathbf{x}^{(K)} \Bigr)
\otimes
  \chi_{\geq}\bigl(  {\bigplus_M}_{W\cap\mathrm{supp}(\mathdutchcal{u})} \mathbf{x}^{(M)}, c \bigr)
\Biggr)
\]
...
\[
[\mathdutchcal{u} \oslash c]_{<}= \bigoplus_{X} \mathbf{x}^{(K)} \ominus \biggl(\bigoplus_{W, |W|=c}\bigotimes \mathbf{x}^{(K)}\biggr)
\]
