...
\paragraph{Semantic Partitioning of Tuples}

Let $\mathbf{X}$ be the universe of tuples (variables). Fix a set of attributes $A$ (one or more) that define semantic similarity. Let
$\phi: \mathbf{X} \to \mathbb{R}^d$
...
be a deterministic clustering. We define a map for assigning blocks from a finite collection $\mathcal{B}$ to $\mathbf{x}\in \mathbf{X}$ as,
\[
\kappa: \mathbf{X} \to \mathcal{B} = \{B_1, \dots, B_m\}, \quad \kappa(\mathbf{x}) = B_{g(\phi(\mathbf{x}))}
\]
...
\]
\color{red}
\paragraph{Choices for $\phi$ and $g$ (From related works on clustering)}
