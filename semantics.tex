...
For the \textbf{equal} case, we have:
\[
[\mathdutchcal{u} \oslash c]_{=}=
\bigoplus_{W,|W| = c} \Biggl(\biggl(\bigotimes_{W\cap \mathrm{supp}(\mathdutchcal{u})}\mathbf{x}^{(K)}\biggr)\otimes \Biggl(\mathbb{1}_K\ominus \biggl(\bigoplus_{(X\setminus W)\cap \mathrm{supp}(\mathdutchcal{u})}\mathbf{x}^{(K)}\biggr) \Biggr)\Biggr)
\]
\begin{theorem}
...
$K$ satisfies $(A9)-(A12)$.
  ...
  If $op$ is $=$,\\
  \begin{multline*}
    [\mathdutchcal{u} \oslash \mathdutchcal{v}]_{=}=
\bigoplus_{W} 
\Biggl(
  \Bigl( \bigotimes_{W\cap\mathrm{supp}(\mathdutchcal{u})} \mathbf{x}^{(K)} \Bigr) \;\otimes\; \Bigl( \bigotimes_{W\cap\mathrm{supp}(\mathdutchcal{v})} \mathbf{x}^{(K)} \Bigr)
\otimes
  \chi_{=}\bigl(  {\bigplus_M}_{W\cap\mathrm{supp}(\mathdutchcal{u})} \mathbf{x}^{(M)}, {\bigplus_M}_{W\cap\mathrm{supp}(\mathdutchcal{v})} \mathbf{x}^{(M)}   \bigr)
\;\otimes\;\\
  \Biggl(\mathbb{1}_K \ominus \Biggl( \bigoplus_{(\mathbf{X}\setminus W)\cap\mathrm{supp}(\mathdutchcal{u})} \mathbf{x}^{(K)} \;\oplus\; \bigoplus_{(\mathbf{X}\setminus W)\cap\mathrm{supp}(\mathdutchcal{u})} \mathbf{x}^{(K)} \Biggr)\Biggr)
\Biggr).
\end{multline*}
...
Thus, 
  \[
   [\mathdutchcal{u} \oslash \mathdutchcal{v}]_{=}= \bigoplus_{W\in\mathcal{W}_c} \Biggl(
