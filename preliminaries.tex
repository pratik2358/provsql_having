...
For the semirings in Example~\ref{ex:semirings}, the monus operation $\ominus$ is defined as follows: for $\mathbb N$ $a\ominus b = \max(a-b,0)$, for $\mathbb B$ $a\ominus b = a\land \lnot b$, and for the Why-provenance semiring  $A\ominus B = A\setminus B$.

Some semirings have additional properties. We list the main properties that will be useful for our results:
\begin{itemize}
...
Table~\ref{tab:semirings} summarize the $\oplus$ operator and whether they respect the properties listed above for the semirings in Example~\ref{ex:semirings} and some additional ones.

We show that in an $m$-semiring, additive idempotence is equivalent to $\ominus$ being right-distributive over $\oplus$.
